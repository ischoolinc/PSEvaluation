...
namespace JHEvaluation.StudentScoreSummaryReport
{
    public partial class PrintForm_StudentScoreCertificattion_English : BaseForm, IStatusReporter
    {
        ...
        public PrintForm_StudentScoreCertificattion_English(List<string> studentIds)
        {
            InitializeComponent();
                            ...
                            if (jssr.SchoolYear == schoolyear_grade_dict[grade])
                            {
                                foreach (var subjectscore in jssr.Subjects)
                                {
                                    ...
                                    if (subjectscore.Value.Domain == "彈性課程" || subjectscore.Value.Domain == "彈性學習" || subjectscore.Value.Domain == "")
                                    {
                                        ...
                                        row["彈性課程" + AlternativeCourse + "_科目名稱"] = _SubjDomainEngNameMapping.GetSubjectEngName(subjectscore.Value.Subject);
                ...
                if (urr_dict.ContainsKey(stuID))
                {
                    foreach (K12.Data.UpdateRecordRecord urr in urr_dict[stuID])
                    {
                        ...
                        if (urr.UpdateCode == "1")
                        {
                            ...
                            //英文版格式  August 29, 2016
                            // 如果不加new CultureInfo("en-US") 資訊 會被轉成中文 "八月"
                            row["入學年月"] = enterday.ToString("MMMM dd ,yyyy",new CultureInfo("en-US"));
                        }
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            document= new Aspose.Words.Document(new System.IO.MemoryStream(Properties.Resources.康橋新竹國小在校成績證明書_英文版_功能變數));

            System.Windows.Forms.SaveFileDialog sd = new System.Windows.Forms.SaveFileDialog();
            sd.Title = "另存新檔";
            sd.FileName = "在校成績證明書(英文)合併欄位總表" + ".doc";
